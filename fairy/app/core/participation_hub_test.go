...
func TestParticipationHubPublishesByConversation(t *testing.T) {
	...
	hub.Publish(api.ParticipationEvent{ConversationID: "c2", Action: "silent"})
	hub.Publish(api.ParticipationEvent{ConversationID: "c1", Action: "wait"})
	select {
...
func TestParticipationHubFailsSlowSubscriber(t *testing.T) {
	...
	for generation := uint64(1); generation <= participationHubBuffer+1; generation++ {
		hub.Publish(api.ParticipationEvent{ConversationID: "c1", Generation: generation})
	}
