...
func TestPromptProjectionCodecRoundTrip(t *testing.T) {
	...
	decoded, err := DecodePromptProjection(encoded)
	if err != nil {
		t.Fatal(err)
	}
	if decoded.Version != PromptProjectionVersion ||
...
func TestPromptProjectionCodecRejectsInvalidStates(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			_, err := DecodePromptProjection([]byte(test.encoded))
...
func TestApplyPromptProjectionOmitsOnlyActiveProjectionRows(t *testing.T) {
	...
	got := applyPromptProjection(messages, PromptProjectionState{
		Version: PromptProjectionVersion,
		Omissions: []PromptProjectionOmission{{
			StartMessageSequence: 1, EndMessageSequence: 2,
			Reason: "memory_committed", MemoryID: "memory-1",
