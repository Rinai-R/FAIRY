...
type VectorOperationMetrics struct {
...
type VectorMetricsSnapshot struct {
	...
	Operations []VectorOperationMetrics `json:"operations"`
}
...
type clientMetrics struct {
	...
	requests   uint64
	errors     uint64
	operations map[string]*VectorOperationMetrics
}

func newClientMetrics() *clientMetrics {
	return &clientMetrics{operations: make(map[string]*VectorOperationMetrics)}
}

func (c *VectorClient) observe(operation string, started time.Time, err error) {
	if c == nil {
		return
	}
	if c.metrics == nil {
	...
	if metric == nil {
		metric = &VectorOperationMetrics{Operation: operation}
...
func (c *VectorClient) Metrics(ctx context.Context) (VectorMetricsSnapshot, error) {
	status, err := c.VerifyCollection(ctx)
	if err != nil {
		return VectorMetricsSnapshot{}, err
	}
	snapshot := VectorMetricsSnapshot{PointCount: status.PointsCount, Operations: []VectorOperationMetrics{}}
	...
	snapshot.Operations = make([]VectorOperationMetrics, 0, len(c.metrics.operations))
