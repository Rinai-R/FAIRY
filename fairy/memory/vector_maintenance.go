...
func (c *VectorClient) ListPoints(ctx context.Context) (points []VectorStoredPoint, err error) {
	started := time.Now()
	...
	points = make([]VectorStoredPoint, 0)
	for {
...
func (c *VectorClient) DeletePoints(ctx context.Context, ids []uuid.UUID) (err error) {
	started := time.Now()
...
func decodeStoredPoint(point *qdrant.RetrievedPoint) (VectorStoredPoint, error) {
	if point == nil || point.GetId() == nil {
		return VectorStoredPoint{}, errors.New("qdrant stored point is missing point id")
	}
	pointID, err := uuid.Parse(point.GetId().GetUuid())
	if err != nil {
		return VectorStoredPoint{}, errors.New("qdrant stored point id is invalid")
	}
	payload := point.GetPayload()
	stored := VectorStoredPoint{
		PointID:     pointID,
	...
	if _, err := VectorPointPayload(VectorPointPayloadInput{ItemKind: stored.ItemKind, ItemID: stored.ItemID, ModelID: stored.ModelID, ScopeType: stored.ScopeType, CharacterID: stored.CharacterID, ContentHash: stored.ContentHash}); err != nil {
		return VectorStoredPoint{}, fmt.Errorf("qdrant stored point payload is invalid: %w", err)
	}
	expected, err := VectorPointID(stored.ItemKind, stored.ItemID, stored.ModelID)
	if err != nil || expected != stored.PointID {
		return VectorStoredPoint{}, errors.New("qdrant stored point id does not match payload")
	}
	return stored, nil
}
