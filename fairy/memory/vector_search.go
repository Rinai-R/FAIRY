...
func (c *VectorClient) Search(ctx context.Context, vector []float32, modelID, characterID string, limit int) (hits []VectorSearchHit, err error) {
	started := time.Now()
	...
	hits = make([]VectorSearchHit, 0, len(points))
...
func decodeSearchHit(point *qdrant.ScoredPoint) (VectorSearchHit, error) {
	if point == nil || point.GetId() == nil {
		return VectorSearchHit{}, errors.New("qdrant search hit is missing point id")
	}
	pointID, err := uuid.Parse(point.GetId().GetUuid())
	if err != nil {
		return VectorSearchHit{}, errors.New("qdrant search hit point id is invalid")
	}
	...
	if _, err := VectorPointPayload(VectorPointPayloadInput{ItemKind: hit.ItemKind, ItemID: hit.ItemID, ModelID: hit.ModelID, ScopeType: hit.ScopeType, CharacterID: hit.CharacterID, ContentHash: hit.ContentHash}); err != nil {
		return VectorSearchHit{}, fmt.Errorf("qdrant search hit payload is invalid: %w", err)
	}
	expectedID, err := VectorPointID(hit.ItemKind, hit.ItemID, hit.ModelID)
	if err != nil || expectedID != hit.PointID {
		return VectorSearchHit{}, errors.New("qdrant search hit point id does not match payload")
	}
	return hit, nil
}
