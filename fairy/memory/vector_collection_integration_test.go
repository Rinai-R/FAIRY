...
func TestQdrantCollectionMigrateVerifyAndRejectIncompatible(t *testing.T) {
	...
	if status.CollectionName != client.config.collectionName() || status.Dimensions != VectorDimensions || status.Distance != VectorDistance {
...
func TestQdrantSearchFiltersRelationshipScope(t *testing.T) {
	...
	vector[0] = 1
	inputs := []VectorPointPayloadInput{
	...
	for _, input := range inputs {
		id, err := VectorPointID(input.ItemKind, input.ItemID, input.ModelID)
		if err != nil {
			t.Fatal(err)
		}
		if err := client.Upsert(ctx, VectorPoint{ID: id, Vector: vector, Payload: input}); err != nil {
			t.Fatal(err)
		}
	}
	...
	deleteID, err := VectorPointID(ItemKindPersonalMemory, "character-b", "model-1")
	if err != nil {
		t.Fatal(err)
	}
...
func TestQdrantMetricsConcurrentUpsertAndSnapshot(t *testing.T) {
	...
	for worker := 0; worker < 8; worker++ {
		...
		go func() {
			...
			pointID, err := VectorPointID(ItemKindPersonalMemory, itemID, "model-1")
			if err != nil {
				t.Error(err)
				return
			}
			if err := client.Upsert(ctx, VectorPoint{ID: pointID, Vector: vector, Payload: VectorPointPayloadInput{
...
func openTestQdrantClient(t *testing.T, ctx context.Context, collection string) *VectorClient {
	t.Helper()
	url := os.Getenv("FAIRY_TEST_QDRANT_GRPC_URL")
	if url == "" {
		url = "http://127.0.0.1:16334"
	}
	client, err := OpenVectorClient(ctx, VectorConfig{URL: url, Timeout: 5 * time.Second, CollectionName: collection})
	if err != nil {
		t.Fatal(err)
	}
	return client
