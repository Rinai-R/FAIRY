...
func TestPointPayloadUsesMinimalMetadataOnly(t *testing.T) {
	payload, err := VectorPointPayload(VectorPointPayloadInput{
...
func TestPointPayloadRejectsInvalidHashAndScope(t *testing.T) {
	base := VectorPointPayloadInput{ItemKind: ItemKindKnowledge, ItemID: "knowledge-1", ModelID: "model", ScopeType: "global", ContentHash: strings.Repeat("a", 64)}
	badHash := base
	badHash.ContentHash = strings.Repeat("A", 64)
	if _, err := VectorPointPayload(badHash); err == nil {
		t.Fatal("VectorPointPayload(bad hash) error = nil")
	}
	badScope := base
	badScope.ScopeType = " global"
	if _, err := VectorPointPayload(badScope); err == nil {
		t.Fatal("VectorPointPayload(bad scope) error = nil")
	}
}

func TestValidateVectorRejectsDimensionsAndNonFiniteValues(t *testing.T) {
	if err := ValidateVector(make([]float32, VectorDimensions)); err != nil {
		t.Fatalf("ValidateVector(valid) error = %v", err)
	}
	if err := ValidateVector(make([]float32, VectorDimensions-1)); err == nil {
		t.Fatal("ValidateVector(wrong dimensions) error = nil")
	}
	nonFinite := make([]float32, VectorDimensions)
