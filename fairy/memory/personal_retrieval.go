...
func ScanRetrievedPersonalMemories(rows pgx.Rows, remaining *int) ([]RetrievedPersonalMemory, error) {
	defer rows.Close()
...
func RetrievePersonalExtractionProjection(
	...
	for _, fragment := range projection {
		...
		if projectionRunes > MaxFTSQueryChars {
