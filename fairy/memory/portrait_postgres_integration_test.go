...
func TestPostgresCompanionPortraitIsScopedAndBounded(t *testing.T) {
	...
	for _, input := range []struct {
		...
		{kind: "profile", scope: MemoryScope{Type: "global"}, content: "用户喜欢先听完再回应"},
		{kind: "preference", scope: MemoryScope{Type: "global"}, content: "情绪低落时先陪伴"},
		{kind: "preference", scope: MemoryScope{Type: "global"}, content: "不喜欢被催促"},
		{kind: "preference", scope: MemoryScope{Type: "global"}, content: "第三条偏好不应进入每 kind 两条上限"},
		{kind: "relationship", scope: MemoryScope{Type: "character", CharacterID: "portrait-character-1"}, content: "和角色一有稳定信任"},
		{kind: "relationship", scope: MemoryScope{Type: "character", CharacterID: "portrait-character-2"}, content: "和角色二的关系不得混入"},
	} {
