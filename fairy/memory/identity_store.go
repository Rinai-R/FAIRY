...
func NewIdentityStore(pool *coredb.Pool) (*IdentityStore, error) {
	if pool == nil || pool.Raw() == nil {
		return nil, ErrIdentityDatabasePoolRequired
	}
	return &IdentityStore{pool: pool, now: time.Now}, nil
}

func (s *IdentityStore) BindOwner(namespace, principalDigest string) error {
	return s.BindOwnerContext(context.Background(), namespace, principalDigest)
}

func (s *IdentityStore) BindOwnerContext(ctx context.Context, namespace, principalDigest string) error {
...
func (s *IdentityStore) ListOwners() ([]OwnerIdentity, error) {
	return s.ListOwnersContext(context.Background())
}

func (s *IdentityStore) ListOwnersContext(ctx context.Context) ([]OwnerIdentity, error) {
	queryCtx, cancel := s.pool.QueryContext(ctx)
	defer cancel()
...
func (s *IdentityStore) UnbindOwner(namespace, principalDigest string) error {
	return s.UnbindOwnerContext(context.Background(), namespace, principalDigest)
}

func (s *IdentityStore) UnbindOwnerContext(ctx context.Context, namespace, principalDigest string) error {
	if err := validateIdentity(namespace, principalDigest); err != nil {
		return err
	}
	queryCtx, cancel := s.pool.QueryContext(ctx)
...
func (s *IdentityStore) IsOwner(namespace, principalDigest string) (bool, error) {
	return s.IsOwnerContext(context.Background(), namespace, principalDigest)
}

func (s *IdentityStore) IsOwnerContext(ctx context.Context, namespace, principalDigest string) (bool, error) {
	if err := validateIdentity(namespace, principalDigest); err != nil {
		return false, err
	}
	queryCtx, cancel := s.pool.QueryContext(ctx)
