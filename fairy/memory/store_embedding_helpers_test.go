...
func TestEmbeddingForContentDisabled(t *testing.T) {
	value, err := embeddingForContent(nil, "正文")
	if err != nil {
	...
	if err := value.Validate(); err != nil {
		t.Fatalf("disabled EmbeddingValue.Validate() error = %v", err)
	}
}
...
func TestEmbeddingForContentBuildsPostgresValue(t *testing.T) {
	...
	value, err := embeddingForContent(embedder, "topic\nstatement")
	if err != nil {
...
func TestEmbeddingForContentRejectsInvalidProviderResults(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			embedder: &fixedSemanticEmbedder{ready: true, dims: SemanticEmbeddingDimensions - 1},
			want:     "dimensions",
		},
		{
			name:     "provider error",
			embedder: &fixedSemanticEmbedder{ready: true, dims: SemanticEmbeddingDimensions, err: errors.New("provider failed")},
			want:     "provider failed",
		},
		{
			name:     "zero results",
			embedder: &fixedSemanticEmbedder{ready: true, dims: SemanticEmbeddingDimensions},
			want:     "result count",
		},
		{
			...
			embedder: &fixedSemanticEmbedder{
				...
				dims:  SemanticEmbeddingDimensions,
				vectors: [][]float32{
					make([]float32, SemanticEmbeddingDimensions),
					make([]float32, SemanticEmbeddingDimensions),
				},
			},
			want: "result count",
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			_, err := embeddingForContent(test.embedder, "content")
