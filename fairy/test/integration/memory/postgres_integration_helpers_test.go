...
func appendModelUsageEvent(t *testing.T, store *memoryIntegrationStores, conversationID, turnID, lane string, input, output uint64, cached *uint64) {
	t.Helper()
...
func appendTerminalEvent(t *testing.T, store *memoryIntegrationStores, conversationID, turnID, status string) {
	t.Helper()
