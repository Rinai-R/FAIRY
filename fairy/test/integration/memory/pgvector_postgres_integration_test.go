...
func (*mappedSemanticEmbedder) Status() embedding.SemanticStatus {
...
func (*mappedSemanticEmbedder) Dims() int { return embedding.Dimensions }
...
func TestPostgresPgvectorLifecycleHybridRecallAndPublicIsolation(t *testing.T) {
	...
	store, err := newMemoryIntegrationStoresWithEmbedder(pool, embedder)
	if err != nil {
		t.Fatal(err)
	}
	...
	personalA, err := store.CreatePersonalMemoryContext(ctx, "relationship", personal.Scope{Type: "character", CharacterID: "character-vector-a"}, "私人向量目标", 9200)
	if err != nil {
		t.Fatal(err)
	}
	personalB, err := store.CreatePersonalMemoryContext(ctx, "relationship", personal.Scope{Type: "character", CharacterID: "character-vector-b"}, "其他角色私人目标", 9100)
	if err != nil {
		t.Fatal(err)
	}
	knowledgeRecord, err := store.InsertVerifiedKnowledgeContext(ctx, "公开主题", "公开向量事实", conversationA, turnA, 9000, nil)
	if err != nil {
		t.Fatal(err)
	}
	assertPostgresEmbedding(t, ctx, pool, "personal_memories", personalA.ID, personalA.Content, true)
	assertPostgresEmbedding(t, ctx, pool, "knowledge_entries", knowledgeRecord.ID, knowledgeRecord.Topic+"\n"+knowledgeRecord.Statement, true)
	...
	if private.SemanticStatus != string(embedding.SemanticStatusUsed) {
	...
	if len(private.Knowledge) == 0 || private.Knowledge[0].ID != knowledgeRecord.ID {
	...
	if len(public.Knowledge) == 0 || public.Knowledge[0].ID != knowledgeRecord.ID {
		t.Fatalf("public knowledge recall = %#v", public.Knowledge)
	}
	ingestRecall, err := store.SearchKnowledgeForIngestContext(
		ctx,
		"语义查询",
		knowledge.MaxSearchCandidates,
	)
	if err != nil {
		t.Fatal(err)
	}
	if len(ingestRecall) == 0 || ingestRecall[0].ID != knowledgeRecord.ID {
	...
	if err := store.TombstoneKnowledgeContext(ctx, knowledgeRecord.ID); err != nil {
		t.Fatal(err)
	}
...
func TestPostgresPgvectorProviderFailureWritesNothing(t *testing.T) {
	...
	store, err := newMemoryIntegrationStoresWithEmbedder(pool, &mappedSemanticEmbedder{err: errors.New("provider unavailable")})
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := store.CreatePersonalMemoryContext(ctx, "preference", personal.Scope{Type: "global"}, "不能落库", 9000); err == nil {
...
func TestPostgresPgvectorMutationWritesNativeV2Only(t *testing.T) {
	...
	personal, err := store.CreatePersonalMemoryContext(ctx, "preference", personal.Scope{Type: "global"}, "原始记忆", 9000)
	if err != nil {
		t.Fatal(err)
	}
...
func TestPostgresPgvectorModelSwitchKeepsTextButIsolatesOldVectors(t *testing.T) {
	...
	currentStore, err := newMemoryIntegrationStoresWithEmbedder(pool, currentEmbedder)
	if err != nil {
		t.Fatal(err)
	}
	seedCompletedTurn(t, ctx, currentStore, "character-model-switch")
	wrongModel, err := currentStore.CreatePersonalMemoryContext(ctx, "preference", personal.Scope{Type: "global"}, "模型切换文本 错模型", 9000)
	if err != nil {
		t.Fatal(err)
	}
	...
	textStore, err := newMemoryIntegrationStores(pool)
	if err != nil {
		t.Fatal(err)
	}
	legacy, err := textStore.CreatePersonalMemoryContext(ctx, "profile", personal.Scope{Type: "global"}, "模型切换文本 旧向量", 8900)
	if err != nil {
		t.Fatal(err)
	}
...
WHERE id = $1`, legacy.ID, embedding.ContentHash(legacy.Content), legacyVector.String()); err != nil {
		t.Fatal(err)
	}
	...
	if result.SemanticStatus != string(embedding.SemanticStatusReady) {
...
func TestPostgresPgvectorSameModelEndpointSwitchUsesDistinctSpace(t *testing.T) {
	...
	store, err := newMemoryIntegrationStoresWithEmbedder(pool, &mappedSemanticEmbedder{
	...
	memory, err := store.CreatePersonalMemoryContext(ctx, "preference", personal.Scope{Type: "global"}, "同名模型端点切换", 9000)
	if err != nil {
		t.Fatal(err)
	}
...
func TestPostgresPgvectorRebuildRepairsMissingVectors(t *testing.T) {
	...
	personal, err := textStore.CreatePersonalMemoryContext(ctx, "profile", personal.Scope{Type: "global"}, "待重建记忆", 9000)
	if err != nil {
		t.Fatal(err)
	}
	...
	vectorStore, err := newMemoryIntegrationStoresWithEmbedder(pool, embedder)
	if err != nil {
		t.Fatal(err)
	}
...
WHERE id = $1`, personal.ID, embedding.ContentHash(personal.Content), legacyV1.String()); err != nil {
		t.Fatal(err)
	}
...
FROM personal_memories WHERE id = $1`, personal.ID).Scan(&legacyAfter, &currentModel, &currentHash); err != nil {
	...
	if legacyAfter != legacyV1.String() || currentModel != testSemanticEmbeddingModelID || currentHash != embedding.ContentHash(personal.Content) {
...
func seedCompletedTurn(t *testing.T, ctx context.Context, store *memoryIntegrationStores, characterID string) (string, string) {
	t.Helper()
