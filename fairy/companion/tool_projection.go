...
type retrievalToolResultSummary struct {
...
type runtimeToolDetail struct {
	Version       string                       `json:"version"`
	Arguments     runtimeToolArguments         `json:"arguments"`
	Receipt       retrievalToolResultSummary   `json:"receipt"`
...
type runtimeToolArguments struct {
...
func retrievalToolResultSummaryFor(status string, result memory.RetrievalContext) retrievalToolResultSummary {
	summary := retrievalToolResultSummary{
...
func runtimeRetrievalToolDetail(query, status string, result, merged memory.RetrievalContext) runtimeToolDetail {
	detail := runtimeToolDetail{
		Version:   "v1",
		Arguments: runtimeToolArguments{Query: runtimeInspectionText(query, maxToolQueryRunes)},
		Receipt:   retrievalToolResultSummaryFor(status, result),
	}
...
func runtimeToolRetrievalProjection(context memory.RetrievalContext) runtimeToolRetrievalContext {
	...
	for _, item := range context.PersonalMemories {
		projected.PersonalMemories = append(projected.PersonalMemories, runtimeToolPersonalMemory{
			...
			Content:               runtimeInspectionText(item.Content, memory.MaxPersonalMemoryContentRunes),
	...
	for _, item := range context.Knowledge {
		...
		for _, source := range item.Sources {
			knowledge.Sources = append(knowledge.Sources, runtimeToolSource{
				...
				Snippet: runtimeInspectionText(source.Snippet, memory.MaxKnowledgeIngestSnippetRunes),
	...
	for _, item := range context.SocialMemories.Entries {
		projected.SocialMemories.Entries = append(projected.SocialMemories.Entries, runtimeToolSocialMemory{
			ID: runtimeInspectionText(item.ID, 128), Kind: runtimeInspectionText(item.Kind, 64),
			Situation:       runtimeInspectionText(item.Situation, memory.MaxSocialSituationRunes),
			Content:         runtimeInspectionText(item.Content, memory.MaxSocialContentRunes),
			RecallCue:       runtimeInspectionText(item.RecallCue, memory.MaxSocialRecallRunes),
...
func retrievalToolPromptItems(call model.FunctionCall, status string, result memory.RetrievalContext) []model.PromptItem {
	summary := retrievalToolResultSummaryFor(status, result)
