...
func TestToolContextSegmentsPreservePairingAndOrder(t *testing.T) {
	...
	segments, err := toolContextSegments([]model.PromptItem{
		{Type: model.PromptItemToolCall, ToolCallID: "call-1", ToolName: toolStickerSearch, ToolArguments: `{}`},
	...
	if segments[1].Recoverability != model.ContextRecoverabilityRefetchable ||
		...
		*segments[1].ExpiresAtUnixMS != created.Add(toolResultSegmentTTL).UnixMilli() {
		t.Fatalf("result segment = %#v", segments[1])
	}
	items, err := (persona.ContextProjector{}).ProjectSlotsWithTail(nil, segments)
	if err != nil {
		t.Fatal(err)
	}
...
func TestDesktopToolContextRemainsEphemeral(t *testing.T) {
	items := desktopToolPromptItems("call-vision", `{}`, DesktopToolEvidence{
		MediaType: "image/png", DataURL: "data:image/png;base64,SENSITIVE",
	})
	segments, err := toolContextSegments(items, time.UnixMilli(1000))
	if err != nil {
		t.Fatal(err)
	}
...
func TestToolContextSegmentsRejectMismatchedCallIDs(t *testing.T) {
	_, err := toolContextSegments([]model.PromptItem{
		{Type: model.PromptItemToolCall, ToolCallID: "call-1", ToolName: toolStickerSearch},
