...
func TestReplayFixture(t *testing.T) {
	...
	retrieval := memory.RetrievalContext{Knowledge: []memory.RetrievedKnowledge{{ID: "knowledge-1", Statement: "公开事实"}}}
	slots, err := BuildRespondContextSlotsWithSocial(record, nil, memory.PromptWindowRecord{Revision: 2}, []memory.MessageRecord{{Role: "user", Content: "当前消息", Sequence: 4}}, []VisualState{{ID: "idle", Description: "待机"}}, retrieval, resolved, SocialRespondContext{Intent: intent})
	if err != nil {
		t.Fatal(err)
	}
	...
	metadata := runtimePromptLedgerMetadata(items, slots, memory.PromptWindowRecord{Revision: 2}, nil, nil, retrieval, cacheInput, true)
	...
	reply, err := compileReplyForInteraction(draft, []VisualState{{ID: "idle", Description: "待机"}}, resolved, intent)
