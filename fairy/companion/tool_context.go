...
const toolResultSegmentTTL = 2 * time.Minute

func toolContextSegments(items []model.PromptItem, createdAt time.Time) ([]model.ContextSegment, error) {
	...
	expiresAtMS := createdAt.Add(toolResultSegmentTTL).UnixMilli()
	recoverability := model.ContextRecoverabilityRefetchable
	if call.ToolName == toolDesktopObserve {
	...
	segments := []model.ContextSegment{
		{
			...
			TokenCount: toolPromptItemTokenCount(call), Recoverability: model.ContextRecoverabilityRequired,
		...
		},
		{
			...
			TokenCount: toolPromptItemTokenCount(result), Recoverability: recoverability, Dependencies: []string{callSegmentID},
			ProjectionState: model.ContextProjectionActive,
		},
	}
...
func toolPromptItemTokenCount(item model.PromptItem) uint64 {
