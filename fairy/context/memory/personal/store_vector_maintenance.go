...
func (s *Store) RebuildVectors(ctx context.Context, pageSize int) (VectorRebuildResult, error) {
	...
	if dims := embedder.Dims(); dims != embedding.Dimensions {
		return VectorRebuildResult{}, fmt.Errorf("embedding dimensions = %d, want %d", dims, embedding.Dimensions)
	}
...
LIMIT $3`, lastKind, lastID, limit)
	...
	for rows.Next() {
		...
		item.Current = persistedModelID.Valid &&
			...
			contentHash.Valid &&
			contentHash.String == embedding.ContentHash(item.Content) &&
			vectorPresent
...
func (s *Store) updateRebuiltVector(ctx context.Context, modelID string, item authoritativeVectorItem, vector []float32) (bool, error) {
	value, err := embedding.ForContent(&precomputedSemanticEmbedder{modelID: modelID, vector: vector}, item.Content)
	if err != nil {
		return false, err
	}
	queryCtx, cancel := s.pool.QueryContext(ctx)
...
func (*precomputedSemanticEmbedder) Status() embedding.SemanticStatus {
	return embedding.SemanticStatusReady
}
func (embedder *precomputedSemanticEmbedder) ModelID() string { return embedder.modelID }
func (*precomputedSemanticEmbedder) Dims() int                { return embedding.Dimensions }
