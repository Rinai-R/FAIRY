...
func ScanRetrieved(rows pgx.Rows, remaining *int) ([]Retrieved, error) {
	defer rows.Close()
...
func RetrieveTrigram(ctx context.Context, db Querier, characterID, query string, remaining *int) ([]Retrieved, error) {
...
func RetrieveExtractionProjection(
	ctx context.Context,
	db Querier,
	characterID string,
	projection []string,
...
LIMIT 64`, characterID, queries)
	...
	return ScanRetrieved(rows, remaining)
}
