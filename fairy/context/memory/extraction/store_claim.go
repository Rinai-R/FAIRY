...
func (s *Store) claimExtractionBatchPostgres(ctx context.Context, conversationID string, limit int) (*BatchInput, error) {
...
ORDER BY turn.sequence`, claimID, s.workerID)
	...
	claimed := make([]TurnRow, 0, len(turnIDs))
	...
	input, err := BuildBatchInput(claimID, conversationID, characterID, claimed, func(projection []string, remaining *int) ([]personal.Retrieved, error) {
		return personal.RetrieveExtractionProjection(queryCtx, tx, characterID, projection, remaining, memoryretrieval.NormalizePostgresQuery)
	})
	if err != nil {
		return nil, err
	}
...
WHERE extraction_claim_id = $1
  ...
  AND extraction_lease_owner = $7`,
		batchID, code, embedding.CleanEmbeddingErrorMessage(message), status,
...
func (s *Store) completeExtractionBatchPostgres(ctx context.Context, batchID string) error {
	...
	defer tx.Rollback(queryCtx)
	if err := SucceedBatch(queryCtx, tx, batchID, s.workerID, nowUnixMS()); err != nil {
		return err
	}
	if err := tx.Commit(queryCtx); err != nil {
