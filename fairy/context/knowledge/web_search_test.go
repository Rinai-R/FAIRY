...
func TestNewWebSearchBatchCleansCanonicalizesAndDeduplicatesSources(t *testing.T) {
	...
	batch, err := NewSearchBatch("conversation-1", "turn-1", "call-1", hits, 123)
	if err != nil {
		t.Fatal(err)
	}
...
func TestNewWebSearchBatchKeepsToolCallBoundaries(t *testing.T) {
	...
	first, err := NewSearchBatch("conversation", "turn", "call-a", hits, 1)
	if err != nil {
		t.Fatal(err)
	}
	second, err := NewSearchBatch("conversation", "turn", "call-b", hits, 1)
	if err != nil {
		t.Fatal(err)
	}
	nextTurn, err := NewSearchBatch("conversation", "turn-next", "call-a", hits, 1)
	if err != nil {
		t.Fatal(err)
	}
...
func TestNewWebSearchBatchBoundsAndRejectsInvalidInputs(t *testing.T) {
	if _, err := NewSearchBatch("", "turn", "call", nil, 0); err == nil {
	...
	batch, err := NewSearchBatch("conversation", "turn", "call", hits, 0)
	if err != nil {
		t.Fatal(err)
	}
