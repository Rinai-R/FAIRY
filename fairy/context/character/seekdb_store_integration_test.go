...
func openCharacterSeekDBRuntime(t *testing.T) (*seekdb.Runtime, *sql.DB, seekdb.Config) {
	...
	config := seekdb.Config{
		BinaryPath: binary, LibraryDirs: filepath.SplitList(os.Getenv(seekdb.EnvLibraryPath)),
		DataDir: filepath.Join(t.TempDir(), "seekdb-data"), Address: reserveCharacterLoopbackAddress(t),
		Database: seekdb.DefaultDatabase, User: seekdb.DefaultUser,
