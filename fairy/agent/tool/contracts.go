...
func Specs(webSearchEnabled bool) []model.ToolSpec {
	return SpecsForInteraction(webSearchEnabled, session.Resolved{Memory: session.MemoryPersonal})
}
...
func InstructionsForTools(toolsEnabled bool) string {
	...
	return character.RespondInstructions
}

func ModelDrivenBudget(resolved session.Resolved) int {
...
func InstructionsForInteraction(toolsEnabled bool, resolved session.Resolved) string {
	if !resolved.AllowsPersonalMemory() {
