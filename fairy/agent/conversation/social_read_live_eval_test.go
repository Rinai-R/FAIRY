...
func TestLivePublicSocialMultiStepToolUse(t *testing.T) {
	...
	memoryPort := &socialLearningMemory{retrieved: social.SocialMemoryContext{Entries: []social.SocialMemoryEntry{
		{ID: "ep-1", Kind: social.SocialMemoryEpisode, Situation: "群友讨论求职准备", Content: "大家会交换整理项目经历的建议", RecallCue: "求职实习准备"},
		{ID: "ex-1", Kind: social.SocialMemoryExpression, Situation: "安慰求职焦虑", Content: "先短句接住情绪再轻问一句进展", RecallCue: "求职焦虑安慰"},
		{ID: "bh-1", Kind: social.SocialMemoryBehavior, Situation: "被点名求助时", Content: "先确认问题再给一个可执行小建议", RecallCue: "被点名求助"},
	}}}

	service := newSocialLearningTestService(memoryPort, modelPort)
	tools := tool.SpecsForInteraction(false, publicAmbientResolved())
	instructions := tool.InstructionsForInteraction(true, publicAmbientResolved())
	...
	for _, call := range calls {
		switch call.Name {
		case tool.SocialContextSearch, tool.SocialExpressionSelect:
			sawSocialTool = true
		}
	}
	if !sawSocialTool {
...
func TestLiveSocialToolRetrievalRelevance(t *testing.T) {
	...
	memoryPort := &socialLearningMemory{retrieved: social.SocialMemoryContext{Entries: []social.SocialMemoryEntry{
		{ID: "ex-1", Kind: social.SocialMemoryExpression, Situation: "安慰求职焦虑", Content: "先短句接住情绪再轻问一句进展", RecallCue: "求职焦虑安慰"},
		{ID: "ex-2", Kind: social.SocialMemoryExpression, Situation: "吃饭约饭", Content: "轻松接梗问地点", RecallCue: "约饭"},
		{ID: "ep-1", Kind: social.SocialMemoryEpisode, Situation: "群友讨论求职准备", Content: "大家会交换整理项目经历的建议", RecallCue: "求职实习准备"},
	}}}
