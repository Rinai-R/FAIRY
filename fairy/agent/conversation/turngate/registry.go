...
func New(cancelHook CancelHook) *Registry {
	return &Registry{gates: make(map[string]*turnGate), cancelHook: cancelHook}
}

func (r *Registry) acquire(conversationID string, create bool) *turnGate {
	r.mu.Lock()
	defer r.mu.Unlock()
	if r.gates == nil {
		if !create {
...
func (r *Registry) release(conversationID string, g *turnGate) {
	if r == nil || g == nil {
		return
	}
	g.mu.Lock()
