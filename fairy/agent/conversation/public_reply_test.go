...
func TestValidateReplyForInteractionRejectsPublicMachineSelfIdentity(t *testing.T) {
	...
	for _, text := range tests {
		reply := reply.CompiledReply{Chains: []reply.ReplyChain{{Text: text, VisualState: "idle"}}}
...
func TestValidateReplyForInteractionPreservesPrivateCharacterIdentity(t *testing.T) {
	reply := reply.CompiledReply{Chains: []reply.ReplyChain{{Text: "哼哼，我可是高性能机器人！", VisualState: "happy"}}}
...
func TestPublicReplyShapeContract(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			chains := make([]reply.ReplyChain, 0, tt.count)
			for i := 0; i < tt.count; i++ {
				chains = append(chains, reply.ReplyChain{VisualState: "idle", Text: "接一句"})
			}
			err := validateReplyForInteraction(reply.CompiledReply{Chains: chains}, publicAmbientResolved(), &ReplyIntent{ReplyMode: tt.mode})
			if (err == nil) != tt.valid {
...
func TestPrivateReplyDoesNotApplyPublicShapeContract(t *testing.T) {
	chains := make([]reply.ReplyChain, 0, 6)
	for i := 0; i < 6; i++ {
		chains = append(chains, reply.ReplyChain{VisualState: "idle", Text: "接一句"})
	}
	if err := validateReplyForInteraction(reply.CompiledReply{Chains: chains}, desktopResolved(), &ReplyIntent{ReplyMode: "brief"}); err != nil {
