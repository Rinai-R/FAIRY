...
type MemoryProjectionInput struct {
	...
	ExistingProjection      historyprojection.State
...
type MemoryProjectionPlan struct {
	CandidateCount         int
...
func PlanMemoryProjection(input MemoryProjectionInput) MemoryProjectionPlan {
	result := MemoryProjectionPlan{AfterTokens: input.CurrentTokens}
	...
	candidates := make([]extraction.Coverage, 0, len(input.Coverage))
	...
	slices.SortFunc(candidates, func(left, right extraction.Coverage) int {
		switch {
	...
	for _, candidate := range candidates {
		result.Omissions = append(result.Omissions, historyprojection.Omission{
