...
func TestCurrentTurnL1ProjectionKeepsProviderToolProtocolValid(t *testing.T) {
	...
	first, err := tool.ContextSegments([]model.PromptItem{
	...
	second, err := tool.ContextSegments([]model.PromptItem{
	...
	plan := contextplan.PlanToolResults(contextplan.ToolResultInput{
	...
	projected := contextplan.ApplyToolResultPlan(segments, plan)
	items, err := (character.ContextProjector{}).Project(projected)
	if err != nil {
		t.Fatal(err)
	}
