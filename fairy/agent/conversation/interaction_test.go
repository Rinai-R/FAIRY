...
func TestInteractionMemoryPolicySelectsToolsAndInstructions(t *testing.T) {
	...
	tools := tool.SpecsForInteraction(true, public)
	if len(tools) != 4 || tools[0].Name != tool.PublicMemorySearch || tools[1].Name != tool.SocialContextSearch || tools[2].Name != tool.SocialExpressionSelect || tools[3].Name != tool.WebSearch {
		t.Fatalf("public tools = %#v", tools)
	}
	instructions := tool.InstructionsForInteraction(true, public)
	if strings.Contains(instructions, "personal memories") || !strings.Contains(instructions, tool.PublicMemorySearch) || !strings.Contains(instructions, tool.SocialContextSearch) || !strings.Contains(instructions, tool.SocialExpressionSelect) || !strings.Contains(instructions, "PUBLIC GROUP IDENTITY OVERRIDE") || !strings.Contains(instructions, "high-performance robot") || !strings.Contains(instructions, "Keep emoji light") {
		t.Fatalf("public instructions violate memory policy: %s", instructions)
	}
	privateInstructions := tool.InstructionsForInteraction(true, desktopResolved())
	...
	for _, spec := range tool.SpecsForInteraction(true, desktopResolved()) {
...
func TestStablePrefixAndProfileProjectionFollowResolvedInteraction(t *testing.T) {
	...
	prefix, err := character.BuildStablePrefixItems(record, nil, states)
	if err != nil {
		t.Fatal(err)
	}
	personal, err := character.BuildRespondContextSlots(record, nil, history.PromptWindowRecord{Revision: 1}, nil, states, recall.Context{}, desktopResolved())
	if err != nil {
		t.Fatal(err)
	}
	public, err := character.BuildRespondContextSlots(record, nil, history.PromptWindowRecord{Revision: 1}, nil, states, recall.Context{}, publicAmbientResolved())
	if err != nil {
		t.Fatal(err)
	}
...
func TestPublicPromptAndCompactionOmitPrivateProfile(t *testing.T) {
	...
	states := []reply.VisualState{{ID: "idle", Description: "待机"}}
	public, err := character.BuildRespondInput(record, profileSnapshot, history.PromptWindowRecord{}, nil, states, recall.Context{}, publicAmbientResolved())
	if err != nil {
		t.Fatal(err)
	}
	personal, err := character.BuildRespondInput(record, profileSnapshot, history.PromptWindowRecord{}, nil, states, recall.Context{}, ownerIMResolved())
	if err != nil {
		t.Fatal(err)
	}
	assertPrivateNameProjection(t, public, personal, name)

	messages := []history.MessageRecord{{Role: "user", Content: "消息", Sequence: 1}}
	publicCompact, err := buildCompactInput(record, profileSnapshot, history.PromptWindowRecord{}, messages, states, publicAmbientResolved())
	if err != nil {
		t.Fatal(err)
	}
	personalCompact, err := buildCompactInput(record, profileSnapshot, history.PromptWindowRecord{}, messages, states, desktopResolved())
	if err != nil {
		t.Fatal(err)
	}
...
func TestInteractionBindingCacheBoundsLRUAndRejectsConflict(t *testing.T) {
	cache := interactionruntime.NewBindingCache(2)
...
func TestInteractionBindingCacheReloadsEvictedBindingFromDurableStore(t *testing.T) {
	...
	service := NewService()
	service.interactions = interactionruntime.NewBindingCache(2)
...
func TestInteractionBindingCacheConcurrentAccessRemainsBounded(t *testing.T) {
	const capacity = 8
	cache := interactionruntime.NewBindingCache(capacity)
	binding := publicAmbientBinding()
...
func TestOutputCapabilitiesAreExplicitLiveSessionFacts(t *testing.T) {
	...
	if got := service.outputCapabilities.OwnerCount("conversation-1"); got != 2 {
		t.Fatalf("lease count after same-owner replacement = %d, want 2", got)
	}
	service.UnbindOutputCapabilities("owner-1", "conversation-1")
	if got := service.outputCapabilities.OwnerCount("conversation-1"); got != 1 {
		t.Fatalf("owner lease count after unbind = %d, want 1", got)
	}
	...
	if service.outputCapabilities.ConversationCount() != 0 {
...
func TestOutputCapabilitiesAreClearedWhenCompanionCloses(t *testing.T) {
	...
	if service.OutputCapabilities("conversation-1").Sticker || service.outputCapabilities.ConversationCount() != 0 {
...
func TestOutputCapabilityLeasesBoundReplaceAndRecover(t *testing.T) {
	service := NewService()
	service.outputCapabilities = interactionruntime.NewCapabilityRegistry(2)
	if err := service.BindOutputCapabilities("owner-1", "conversation-1", session.OutputCapabilities{Sticker: true}); err != nil {
		t.Fatal(err)
	}
	if err := service.BindOutputCapabilities("owner-2", "conversation-1", session.OutputCapabilities{}); err != nil {
	...
	if service.outputCapabilities.LeaseCount() != 2 || service.outputCapabilities.ConversationCount() != 1 {
		t.Fatalf("leases=%d conversations=%d, want 2/1", service.outputCapabilities.LeaseCount(), service.outputCapabilities.ConversationCount())
	}
	service.UnbindOutputCapabilities("owner-2", "conversation-1")
	if service.outputCapabilities.LeaseCount() != 1 {
		t.Fatalf("leases after unbind = %d, want 1", service.outputCapabilities.LeaseCount())
	}
	...
	if service.outputCapabilities.LeaseCount() != 2 || service.outputCapabilities.ConversationCount() != 2 {
		t.Fatalf("leases=%d conversations=%d after replacement", service.outputCapabilities.LeaseCount(), service.outputCapabilities.ConversationCount())
	}
	service.clearOutputCapabilities()
	if service.outputCapabilities.LeaseCount() != 0 || service.outputCapabilities.ConversationCount() != 0 {
		t.Fatalf("clear retained leases=%d conversations=%d", service.outputCapabilities.LeaseCount(), service.outputCapabilities.ConversationCount())
	}
}
