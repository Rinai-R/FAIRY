...
func (s *Service) decideContinuation(
	conversationID string,
	cacheRetention bool,
	promptWindowRevision uint64,
	fullRequest model.CompiledPromptRequest,
) (model.ContinuationDecision, *historyruntime.LaneContinuationRecord, error) {
	if !cacheRetention {
		return model.ContinuationDecision{FullReason: model.ContinuationCapabilityUnsupported}, nil, nil
	}
	record, ok, err := s.memory.turn.runtimeState.LoadLaneContinuation(conversationID, string(model.PromptLaneRespond))
	if err != nil {
...
func continuationSuffixFromHashes(record historyruntime.LaneContinuationRecord, input []model.PromptItem) ([]model.PromptItem, bool, bool) {
...
func (s *Service) clearContinuationState(conversationID string) error {
	if s == nil || s.memory.turn.runtimeState == nil {
		return nil
	}
	return s.memory.turn.runtimeState.ClearLaneContinuation(conversationID, string(model.PromptLaneRespond))
}

func (s *Service) updateContinuationState(
	conversationID string,
	cacheRetention bool,
	promptWindowRevision uint64,
	fullRequest model.CompiledPromptRequest,
	...
	_, err := s.memory.turn.runtimeState.SaveLaneContinuation(historyruntime.LaneContinuationRecord{
