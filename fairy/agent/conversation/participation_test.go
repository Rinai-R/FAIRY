...
func TestDeriveRecentPresenceUsesInclusiveWindows(t *testing.T) {
	...
	if _, err := initiative.DeriveRecentPresence([]history.MessageRecord{{Role: "assistant", CreatedAtUnixMS: now + 1}}, now); err == nil {
...
func TestDeriveParticipationSignalsContainsOnlyObjectiveTimingAndPresenceFacts(t *testing.T) {
	...
	signals, err := initiative.DeriveParticipationSignals(messages, []history.MessageRecord{{Role: "assistant", CreatedAtUnixMS: now - 2*int64(time.Minute.Milliseconds())}, {Role: "user", CreatedAtUnixMS: now - int64(time.Minute.Milliseconds())}}, now)
	if err != nil {
...
func TestParticipationActivityMatchesTranscriptSignalsAndPromptTail(t *testing.T) {
	...
	tests := []struct {
		name       string
		transcript []history.MessageRecord
		...
		},
		{
			...
			activity: history.ConversationActivity{UserMessages30Minutes: 1, LastAssistantMessageAtUnixMS: &latestOld},
		},
	}
	for _, test := range tests {
...
func (m *participationMemory) LoadConversationActivity(_ string, nowUnixMS int64) (history.ConversationActivity, error) {
	m.activityLoads++
	activity := history.ConversationActivity{Conversation: m.bootstrap.Conversation}
	...
	for _, message := range m.bootstrap.Messages {
		...
		case "assistant":
			if age < 0 {
				return history.ConversationActivity{}, errors.New("assistant message timestamp is after activity evaluation time")
			}
...
func (m *participationMemory) RetrieveSocialMemoryContext(context.Context, string, string, string) (social.SocialMemoryContext, error) {
	m.retrieveCalls++
	return m.retrieved, nil
}

func (m *participationMemory) RetrieveCharacterSocialMemoryContext(context.Context, string, string) (social.SocialMemoryContext, error) {
	m.retrieveCalls++
	return m.retrieved, nil
}

func (m *participationMemory) ListSocialPersonNotes(context.Context, string, string, []string) ([]social.SocialPersonNote, error) {
	return nil, nil
}
...
func (h participationDecisionHost) LoadConversationActivity(conversationID string, nowUnixMS int64) (history.ConversationActivity, error) {
	if h.service == nil || h.service.memory.ambient.activity == nil {
		return history.ConversationActivity{}, ErrTurnRuntimeUnavailable
	}
...
func (h participationDecisionHost) ListSocialPersonNotes(ctx context.Context, characterID, conversationID string, senderIDs []string) ([]social.SocialPersonNote, error) {
	return h.service.memory.ambient.socialContext.ListSocialPersonNotes(ctx, characterID, conversationID, senderIDs)
}

func (h participationDecisionHost) RetrieveSocialMemoryContext(ctx context.Context, characterID, conversationID, query string) (social.SocialMemoryContext, error) {
...
func TestDecideParticipationRetriesOneInvalidDraftAndAccumulatesUsage(t *testing.T) {
	...
	memoryPort := &participationMemory{bootstrap: history.ConversationBootstrap{Conversation: history.ConversationRecord{ID: "c1", CharacterID: "character-1"}}}
	service := NewService()
...
func TestDecideParticipationSkipsSocialMemoryForPersonalInteraction(t *testing.T) {
	memoryPort := &participationMemory{
		bootstrap: history.ConversationBootstrap{Conversation: history.ConversationRecord{ID: "c1", CharacterID: "character-1"}},
		retrieved: social.SocialMemoryContext{Entries: []social.SocialMemoryEntry{{
			ID: "e1", Kind: social.SocialMemoryBehavior, Situation: "不该出现", Content: "私人路径不得注入", RecallCue: "禁止",
		}}},
	}
	service := NewService()
	service.memory = participationMemoryPorts(memoryPort)
