...
type ParticipationResult struct {
	...
	Usage           []model.LaneModelUsage `json:"-"`
}
...
func DeriveRecentPresence(messages []history.MessageRecord, nowUnixMS int64) (RecentPresence, error) {
	if nowUnixMS <= 0 {
...
func DeriveRecentPresenceFromActivity(activity history.ConversationActivity, nowUnixMS int64) (RecentPresence, error) {
	if nowUnixMS <= 0 {
		return RecentPresence{}, errors.New("presence evaluation time must be positive")
	}
...
func DeriveParticipationSignals(messages []AmbientObservation, transcript []history.MessageRecord, nowUnixMS int64) (ParticipationSignals, error) {
	assistantReplies := 0
	externalMessages := 0
...
func DeriveParticipationSignalsFromActivity(messages []AmbientObservation, activity history.ConversationActivity, nowUnixMS int64) (ParticipationSignals, error) {
...
func BuildParticipationInput(record character.Record, resolved session.Resolved, reason ParticipationEvaluationReason, messages []AmbientObservation, presence RecentPresence) ([]model.PromptItem, error) {
	return BuildParticipationInputWithSignals(record, resolved, reason, messages, nil, presence, time.Now().UnixMilli(), nil)
}

func BuildParticipationInputWithSignals(record character.Record, resolved session.Resolved, reason ParticipationEvaluationReason, messages []AmbientObservation, cacheMessages []AmbientObservation, presence RecentPresence, nowUnixMS int64, transcript []history.MessageRecord) ([]model.PromptItem, error) {
...
func BuildParticipationInputWithActivity(record character.Record, resolved session.Resolved, reason ParticipationEvaluationReason, messages []AmbientObservation, cacheMessages []AmbientObservation, presence RecentPresence, nowUnixMS int64, activity history.ConversationActivity) ([]model.PromptItem, error) {
	return buildParticipationInput(record, resolved, reason, messages, cacheMessages, presence, func() (ParticipationSignals, error) {
		return DeriveParticipationSignalsFromActivity(messages, activity, nowUnixMS)
	})
}

func buildParticipationInput(record character.Record, resolved session.Resolved, reason ParticipationEvaluationReason, messages []AmbientObservation, cacheMessages []AmbientObservation, presence RecentPresence, deriveSignals func() (ParticipationSignals, error)) ([]model.PromptItem, error) {
...
func CompileParticipation(draft string, messages []AmbientObservation) (ParticipationResult, error) {
	...
	case ParticipationReply:
		...
		if !ContainsObservationID(messages, target) {
...
func ContainsObservationID(messages []AmbientObservation, target string) bool {
	for _, message := range messages {
