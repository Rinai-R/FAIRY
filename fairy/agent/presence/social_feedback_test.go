...
func feedbackInputs(host *learningTestHost) []social.SocialFeedbackBatchInput {
	host.mu.Lock()
...
func TestFeedbackZeroObservationAndModelPathsPreserveContract(t *testing.T) {
	...
	if len(feedback) != 1 || len(feedback[0].Evaluations) != 1 || feedback[0].Evaluations[0].Adoption != social.SocialFeedbackUncertain || feedback[0].Evaluations[0].Outcome != social.SocialFeedbackUnknown || feedback[0].ObservedMessageCount != 0 {
...
func TestFeedbackStoreFailureIsReturnedAfterStrictEvaluation(t *testing.T) {
	...
	feedback := feedbackInputs(host)
	if len(feedback) != 1 || feedback[0].Evaluations[0].Outcome != social.SocialFeedbackPartial {
...
func TestFeedbackWindowBoundCloseAndEmptyEntryIDs(t *testing.T) {
	...
	for time.Now().Before(deadline) {
		feedback := feedbackInputs(host)
		if len(feedback) == 1 {
			if len(feedback[0].Evaluations) != 1 || feedback[0].Evaluations[0].Outcome != social.SocialFeedbackNegative || feedback[0].ObservedMessageCount != socialFeedbackObservationLimit {
...
func TestFeedbackEngineRejectsDuplicateTurnWithoutReplacingOwner(t *testing.T) {
	...
	duplicate.Candidates = []social.SocialFeedbackCandidate{{ID: "replacement-entry", Content: "replacement"}}
