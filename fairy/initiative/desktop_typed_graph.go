...
func CompileDesktopTypedGraph(rulebook DesktopRulebook, observation DesktopObservation) (*DesktopTypedGraph, error) {
	...
	b := newDesktopGraphBuilder(MaxDesktopGraphNodes).
		add(
	...
	if rulebook.AttentionBudget <= 0 || observation.Privacy != DesktopPrivacyNormal {
		return b.add(desktopStep("silent", DesktopNodeSilent, func(state DesktopGraphState) DesktopGraphState {
			state.Action = DesktopActionSilent
			return state
	...
	if rulebook.AllowsPlanner && rulebook.AllowsInitiation && observation.Lifecycle == DesktopLifecycleReturned {
		step := desktopGraphStep{key: "initiate", kind: string(DesktopNodeInitiate), run: func(ctx context.Context, state DesktopGraphState) (DesktopGraphState, error) {
			state.Action = DesktopActionInitiate
		...
		b.add(step).path("initiate", "final")
...
func desktopStep(key string, kind DesktopGraphNodeKind, apply func(DesktopGraphState) DesktopGraphState) desktopGraphStep {
	return desktopGraphStep{
		key:  key,
		kind: string(kind),
		run: func(_ context.Context, state DesktopGraphState) (DesktopGraphState, error) {
