...
func TestSaveModelConnectionWritesConfigAndSecret(t *testing.T) {
	...
	value, ok, err := secrets.Load(connection.ConnectionID)
	if err != nil {
...
func TestSaveModelConnectionPreservesExistingSecretWhenKeyOmitted(t *testing.T) {
	...
	apiKey := "sk-test-secret"
	secrets := secret.NewTestStore()
	if _, err := SaveModelConnection(root, ModelConnectionInput{Protocol: "chat_completions", Endpoint: "https://api.deepseek.com", Model: "deepseek-v4-flash", ContextWindowTokens: 1048576, AuthMode: "bearer_key"}, &apiKey, secrets); err != nil {
		t.Fatalf("initial SaveModelConnection() error = %v", err)
	}
	if _, err := SaveModelConnection(root, ModelConnectionInput{Protocol: "responses", Endpoint: "https://api.deepseek.com", Model: "deepseek-v4-flash", ContextWindowTokens: 1048576, AuthMode: "bearer_key"}, nil, secrets); err != nil {
...
func TestSaveModelConnectionNoAuthDeletesExistingSecret(t *testing.T) {
	...
	if _, err := SaveModelConnection(root, ModelConnectionInput{Protocol: "chat_completions", Endpoint: "https://api.deepseek.com", Model: "deepseek-v4-flash", ContextWindowTokens: 1048576, AuthMode: "no_auth"}, nil, secrets); err != nil {
		t.Fatalf("no_auth SaveModelConnection() error = %v", err)
	}
	_, ok, err := secrets.Load(connection.ConnectionID)
	if err != nil {
		t.Fatalf("Load() error = %v", err)
	}
	if ok {
...
func TestClearModelConnectionDeletesConfigAndSecret(t *testing.T) {
	...
	cleared, err := ClearModelConnection(root, secrets)
	if err != nil {
