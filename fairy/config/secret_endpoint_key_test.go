...
func TestDigestEndpointKeyIsStableAndDomainSeparated(t *testing.T) {
	...
	ciphertext, err := SecretCipherFromEnv(func(string) string { return key })
	if err != nil {
		t.Fatal(err)
	}
	first, err := ciphertext.DigestEndpointKey(session.EndpointIM, "onebot-group:123")
	if err != nil {
		t.Fatal(err)
	}
	second, err := ciphertext.DigestEndpointKey(session.EndpointIM, "onebot-group:123")
	if err != nil {
		t.Fatal(err)
	}
	otherEndpoint, err := ciphertext.DigestEndpointKey(session.EndpointDesktop, "onebot-group:123")
	if err != nil {
		t.Fatal(err)
	}
