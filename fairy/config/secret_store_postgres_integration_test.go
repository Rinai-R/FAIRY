...
func TestPostgresStoreEncryptsRoundTripsRotatesNonceAndRejectsWrongKey(t *testing.T) {
	...
	cipher, err := newSecretCipher(bytesOf(1, keyBytes), bytes.NewReader(ascendingBytes(48)))
	if err != nil {
		t.Fatal(err)
	}
	store, err := NewPostgresSecretStore(pool, cipher)
	if err != nil {
		t.Fatal(err)
	}
	value, err := NewSecretValue("sk-live-exact-secret")
	if err != nil {
		t.Fatal(err)
	}
	...
	wrongCipher, err := newSecretCipher(bytesOf(2, keyBytes), bytes.NewReader(bytesOf(4, 12)))
	if err != nil {
		t.Fatal(err)
	}
	wrongStore, err := NewPostgresSecretStore(pool, wrongCipher)
	if err != nil {
		t.Fatal(err)
	}
