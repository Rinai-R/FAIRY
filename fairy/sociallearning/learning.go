...
type LearningSnapshot struct {
	...
	Messages       []participation.AmbientObservation
}
...
type LearningEngine struct {
	...
	queue  *boundedqueue.Queue[LearningSnapshot]
	wg     sync.WaitGroup
	once   sync.Once
	closed atomic.Bool
...
func NewLearningEngine(host LearningHost, capacity int) *LearningEngine {
	...
	engine := &LearningEngine{host: host, ctx: ctx, cancel: cancel, queue: boundedqueue.New[LearningSnapshot](capacity)}
	engine.wg.Add(1)
	go engine.run()
	return engine
}
...
func (e *LearningEngine) Enqueue(snapshot LearningSnapshot) bool {
	...
	snapshot.Messages = append([]participation.AmbientObservation(nil), snapshot.Messages...)
	select {
	case <-e.ctx.Done():
		return false
	default:
...
func buildSocialLearningInput(record character.Record, resolved domain.Resolved, messages []participation.AmbientObservation) ([]model.PromptItem, error) {
...
func compileSocialLearning(draft string, messages []participation.AmbientObservation) (socialLearnCompiled, error) {
	...
	messageByID := make(map[string]participation.AmbientObservation, len(messages))
	for _, message := range messages {
...
func compileSocialLearningPersonNotes(raw json.RawMessage, messages map[string]participation.AmbientObservation) ([]socialLearnCompiledPersonNote, error) {
	if len(raw) == 0 {
		return nil, nil
	}
	if string(raw) == "null" {
...
func compileSocialLearningPersonNote(
	...
	messages map[string]participation.AmbientObservation,
...
func compileSocialLearningEntry(index int, item socialLearnEntryDraft, messages map[string]participation.AmbientObservation) (memory.SocialMemoryEntryInput, error) {
