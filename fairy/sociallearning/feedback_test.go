...
func feedbackTestSnapshot(observationCount int) feedbackSnapshot {
	observations := make([]participation.AmbientObservation, 0, observationCount)
	for index := 0; index < observationCount; index++ {
		observations = append(observations, participation.AmbientObservation{MessageID: "later-message", SenderID: "member-1", SenderName: "群友", Text: "接着聊这个话题", TimestampUnixMS: int64(index + 1)})
	}
...
func TestFeedbackWindowBoundCloseAndEmptyEntryIDs(t *testing.T) {
	...
	for index := 0; index < socialFeedbackObservationLimit; index++ {
		engine.Observe(registration.ConversationID, participation.AmbientObservation{MessageID: "later-" + strings.Repeat("x", index+1), SenderID: "member", SenderName: "群友", Text: "这条回复需要修正", TimestampUnixMS: int64(index + 1)})
	}
