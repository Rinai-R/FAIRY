...
func TestStoreSetRestartAndClear(t *testing.T) {
	...
	same, err := NewProfileStore(root).SetPreferredName(&name)
	if err != nil {
	...
	cleared, err := NewProfileStore(root).Clear()
	if err != nil {
...
func TestStoreClearAbsentDoesNotFabricateProfile(t *testing.T) {
	update, err := NewProfileStore(t.TempDir()).Clear()
	if err != nil {
		t.Fatalf("Clear() error = %v", err)
	}
...
func TestStoreRejectsInvalidPreferredName(t *testing.T) {
	name := "名字\n第二行"
	_, err := NewProfileStore(t.TempDir()).SetPreferredName(&name)
	if err == nil {
