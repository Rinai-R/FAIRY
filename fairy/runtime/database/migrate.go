// Package database owns FAIRY's PostgreSQL connection, migration, and readiness.
package database
...
var postgresConstraints = []schemaConstraint{
	...
	{"prompt_windows", "prompt_windows_projection_check", "CHECK (projection_revision > 0 AND jsonb_typeof(projection_state) = 'object' AND jsonb_typeof(projection_state->'version') = 'number' AND (projection_state->>'version')::bigint = 1 AND jsonb_typeof(projection_state->'omissions') = 'array')"},
	...
	{"memory_context_coverages", "memory_context_coverages_conversation_fk", "FOREIGN KEY (conversation_id) REFERENCES conversations(id) ON DELETE CASCADE"},
	{"memory_context_coverages", "memory_context_coverages_turn_fk", "FOREIGN KEY (turn_id) REFERENCES conversation_turns(id) ON DELETE CASCADE"},
	{"memory_context_coverages", "memory_context_coverages_memory_fk", "FOREIGN KEY (memory_id) REFERENCES personal_memories(id) ON DELETE RESTRICT"},
