...
func requireToolTurn(ctx context.Context, tx pgx.Tx, conversationID, turnID string) error {
	var exists int
...
func (s *Store) createToolExecutionPostgres(ctx context.Context, input CreateToolExecutionInput) (ToolExecutionRecord, error) {
	...
	if err := requireToolTurn(queryCtx, tx, input.ConversationID, input.TurnID); err != nil {
		return ToolExecutionRecord{}, err
	}
