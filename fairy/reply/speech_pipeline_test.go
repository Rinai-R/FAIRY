...
func TestSpeechPipelineEmitsInEnqueueOrder(t *testing.T) {
	...
	pipeline := NewSpeechPipeline(context.Background(), synth, 8, func(res SpeechResult) {
		mu.Lock()
	...
	pipeline.Enqueue(SpeechJob{PlayIndex: 0, ChainIndex: ChainIndexUtterance, Resolve: func() (string, error) { return "等我看看", nil }})
	pipeline.Enqueue(SpeechJob{PlayIndex: 1, ChainIndex: 0, Resolve: func() (string, error) { return "找到了", nil }})
	pipeline.Enqueue(SpeechJob{PlayIndex: 2, ChainIndex: 1, Resolve: func() (string, error) { return "就是这个", nil }})
	pipeline.Close()
...
func TestSpeechPipelineSkipsEmptyResolve(t *testing.T) {
	...
	var mu sync.Mutex
	var results []SpeechResult
	pipeline := NewSpeechPipeline(context.Background(), synth, 4, func(res SpeechResult) {
		mu.Lock()
		results = append(results, res)
		mu.Unlock()
	})
	pipeline.Enqueue(SpeechJob{PlayIndex: 0, ChainIndex: ChainIndexUtterance, Resolve: func() (string, error) { return "", nil }})
	pipeline.Close()
...
func TestSpeechPipelineDoesNotSynthesizeAfterCancel(t *testing.T) {
	...
	var mu sync.Mutex
	var results []SpeechResult
	pipeline := NewSpeechPipeline(ctx, synth, 4, func(res SpeechResult) {
		mu.Lock()
		results = append(results, res)
		mu.Unlock()
	})
	pipeline.Enqueue(SpeechJob{PlayIndex: 0, ChainIndex: 0, Resolve: func() (string, error) { return "取消后不应合成", nil }})
	pipeline.Close()
	if calls.Load() != 0 {
