...
func TestSessionOutputCapabilitiesFollowConnectionLifetime(t *testing.T) {
	companionService := companion.NewCompanionService()
	apiServer := &Server{rt: &Dependencies{Companion: companionService}}
...
func TestSessionOutputCapabilitiesBindRollsBackAfterShutdown(t *testing.T) {
	...
	connection := &sessionConn{
		server:             &Server{rt: &Dependencies{Companion: companionService}},
		ownerID:            "owner-1",
...
func TestSessionConnectionRollsBackAssociationWhenGlobalCapabilityCapacityIsFull(t *testing.T) {
	companionService := companion.NewCompanionService()
	for index := 0; index < companion.OutputCapabilityLeaseCapacity; index++ {
	...
	if !errors.Is(err, companion.ErrOutputCapabilityCapacity) {
