...
func TestBuildInputPreservesCompleteTurnEvidence(t *testing.T) {
	userMessage := strings.Repeat("完整用户证据", 500) + "\n第二行\t末尾"
	assistantMessage := strings.Repeat("完整助手证据", 300) + "\r\n结束"
	batch := memory.ExtractionBatchInput{
		BatchID:        "batch-1",
...
func TestParseMutationOutputRejectsMissingSourceTurnID(t *testing.T) {
	for _, raw := range []string{
		...
		if _, err := ParseMutationOutput(raw); err == nil {
			t.Fatalf("ParseMutationOutput(%s) accepted missing source turn evidence", raw)
		}
	}
}
