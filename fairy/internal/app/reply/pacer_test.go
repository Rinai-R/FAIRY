...
func TestTargetReplyInterval(t *testing.T) {
	tests := []struct {
		...
		{name: "minimum for short CJK", current: "你好", want: PaceMinimum},
		{name: "minimum for short ASCII", current: "hello", want: PaceMinimum},
		{name: "CJK weighted", current: strings.Repeat("好", 20), want: 1620 * time.Millisecond},
		{name: "mixed scripts", current: "好a", want: PaceMinimum},
		...
		{name: "maximum clamp", current: strings.Repeat("好", 100), want: PaceMaximum},
		{name: "whitespace ignored", current: "   \n\t", want: PaceMinimum},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := TargetInterval(tt.previous, tt.current); got != tt.want {
...
func TestReplyPacerWaitsOnlyRemainingBudget(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		wantTarget := TargetInterval("第一拍。", "第二拍")
