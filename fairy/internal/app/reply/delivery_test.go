...
func TestReplyDeliveryPublishesFirstBeatWithoutPaceWait(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		delivery := NewDelivery(t.Context(), 1, func(completion BeatReadyCompletion) error {
			completions = append(completions, completion)
			return nil
		}, func(record DeliveryRecord) {
...
func TestReplyDeliveryPacesSecondBeatAndReportsPrefix(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		var completions []BeatReadyCompletion
		delivery := NewDelivery(t.Context(), 2, func(completion BeatReadyCompletion) error {
			completions = append(completions, completion)
			return nil
		}, nil)
		...
		target := TargetInterval(first.Text, second.Text)
...
func TestReplyDeliveryCancellationKeepsPublishedPrefix(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		var completions []BeatReadyCompletion
		var records []DeliveryRecord
		delivery := NewDelivery(ctx, 2, func(completion BeatReadyCompletion) error {
			completions = append(completions, completion)
			return nil
		}, func(record DeliveryRecord) {
			records = append(records, record)
