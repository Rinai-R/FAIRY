...
func (e *LearningEngine) Enqueue(snapshot LearningSnapshot) bool {
	...
	snapshot.Messages = append([]Observation(nil), snapshot.Messages...)
	select {
	case <-e.ctx.Done():
		return false
	default:
...
func buildSocialLearningInput(record character.Record, resolved domain.Resolved, messages []Observation) ([]model.PromptItem, error) {
...
func compileSocialLearning(draft string, messages []Observation) (socialLearnCompiled, error) {
	...
	messageByID := make(map[string]Observation, len(messages))
	for _, message := range messages {
...
func compileSocialLearningPersonNotes(raw json.RawMessage, messages map[string]Observation) ([]socialLearnCompiledPersonNote, error) {
	if len(raw) == 0 {
		return nil, nil
	}
	if string(raw) == "null" {
...
func compileSocialLearningEntry(index int, item socialLearnEntryDraft, messages map[string]Observation) (memory.SocialMemoryEntryInput, error) {
