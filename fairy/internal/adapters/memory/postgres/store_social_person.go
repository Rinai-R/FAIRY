...
func upsertSocialPersonNotePostgres(ctx context.Context, db RowQuerier, id, characterID, conversationID, senderID, senderName, note string, now int64) (SocialPersonNote, error) {
	return UpsertSocialPersonNote(ctx, db, id, characterID, conversationID, senderID, senderName, note, now)
}

func listSocialPersonNotesPostgres(ctx context.Context, db Querier, characterID, conversationID string, senderIDs []string, limit int) ([]SocialPersonNote, error) {
	return ListSocialPersonNotes(ctx, db, characterID, conversationID, senderIDs, limit)
}
