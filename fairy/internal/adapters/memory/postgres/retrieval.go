...
func BuildFTSQuery(query string) (string, error) {
	if len([]rune(query)) > domainmemory.MaxFTSQueryChars {
...
func RetrieveKnowledgeTrigram(ctx context.Context, db Querier, query string, remaining *int) ([]domainmemory.RetrievedKnowledge, error) {
...
LIMIT $2`, query, maxResultsPerKind)
	...
	defer rows.Close()
	results := make([]domainmemory.RetrievedKnowledge, 0)
	for rows.Next() {
		var record domainmemory.RetrievedKnowledge
		var confidence int
