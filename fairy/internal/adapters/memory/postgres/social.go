...
func ScanSocialMemoryEntry(row scanner) (domainmemory.SocialMemoryEntry, error) {
	var entry domainmemory.SocialMemoryEntry
	if err := row.Scan(
		...
		return domainmemory.SocialMemoryEntry{}, fmt.Errorf("scanning social memory entry: %w", err)
	}
	if !domainmemory.ValidSocialMemoryKind(entry.Kind) || (entry.Status != "active" && entry.Status != "suppressed") {
		return domainmemory.SocialMemoryEntry{}, errors.New("stored social memory entry is invalid")
	}
	return entry, nil
}

func SocialMemoryContentHash(entry domainmemory.SocialMemoryEntryInput) string {
...
func InsertSocialMemoryEntry(
	...
	candidate domainmemory.SocialMemoryEntryInput,
	now int64,
) (domainmemory.SocialMemoryEntry, error) {
...
func QuerySocialMemoryContext(ctx context.Context, db Querier, characterID, conversationID string, queryFragments []string) (domainmemory.SocialMemoryContext, error) {
...
LIMIT 12`, characterID, conversationID, queryFragments)
	if err != nil {
		return domainmemory.SocialMemoryContext{}, fmt.Errorf("querying social memory: %w", err)
	}
	defer rows.Close()
	return collectSocialMemoryContext(rows, 9, 1800)
}

func QueryCharacterSocialMemoryContext(ctx context.Context, db Querier, characterID string, queryFragments []string) (domainmemory.SocialMemoryContext, error) {
	rows, err := db.Query(ctx, `
SELECT entry.id, entry.character_id, entry.conversation_id, entry.kind, entry.situation, entry.content, entry.recall_cue, entry.status,
       entry.source_start_ms, entry.source_end_ms, entry.use_count, entry.positive_count, entry.negative_count, entry.unknown_count,
       entry.created_at_ms, entry.updated_at_ms
...
LIMIT 24`, characterID, queryFragments)
	if err != nil {
		return domainmemory.SocialMemoryContext{}, fmt.Errorf("querying character social memory: %w", err)
	}
	defer rows.Close()
	return collectSocialMemoryContext(rows, 12, 1800)
}

func collectSocialMemoryContext(rows pgx.Rows, capacity, runeBudget int) (domainmemory.SocialMemoryContext, error) {
	entries := make([]domainmemory.SocialMemoryEntry, 0, capacity)
	perKind := make(map[string]int, 3)
	remaining := runeBudget
	for rows.Next() {
		entry, scanErr := ScanSocialMemoryEntry(rows)
		if scanErr != nil {
			return domainmemory.SocialMemoryContext{}, scanErr
		}
	...
	if err := rows.Err(); err != nil {
		return domainmemory.SocialMemoryContext{}, fmt.Errorf("iterating social memory: %w", err)
	}
	return domainmemory.SocialMemoryContext{Entries: entries}, nil
}

func RecordSocialReplyFeedback(
	ctx context.Context,
	tx pgx.Tx,
	input domainmemory.SocialReplyFeedbackInput,
	id string,
	now int64,
	negativeSuppressThreshold int,
) (domainmemory.SocialReplyFeedback, error) {
	var turnCount int
	if err := tx.QueryRow(ctx, "SELECT COUNT(*) FROM conversation_turns WHERE id = $1 AND conversation_id = $2", input.TurnID, input.ConversationID).Scan(&turnCount); err != nil {
		return domainmemory.SocialReplyFeedback{}, fmt.Errorf("checking social feedback turn: %w", err)
	}
	if turnCount != 1 {
		return domainmemory.SocialReplyFeedback{}, errors.New("social feedback turn does not belong to the conversation")
	}
	if len(input.EntryIDs) > 0 {
		...
		if err := tx.QueryRow(ctx, "SELECT COUNT(*) FROM social_memory_entries WHERE character_id = $1 AND conversation_id = $2 AND id = ANY($3)", input.CharacterID, input.ConversationID, input.EntryIDs).Scan(&entryCount); err != nil {
			return domainmemory.SocialReplyFeedback{}, fmt.Errorf("checking social feedback entries: %w", err)
		}
		if entryCount != len(input.EntryIDs) {
			return domainmemory.SocialReplyFeedback{}, errors.New("social feedback entries do not belong to the conversation")
		}
	}
	...
	if err != nil {
		return domainmemory.SocialReplyFeedback{}, fmt.Errorf("serializing social feedback entry IDs: %w", err)
	}
...
INSERT INTO social_reply_feedback(
    ...
    observed_message_count, created_at_ms
		...
		return domainmemory.SocialReplyFeedback{}, fmt.Errorf("inserting social feedback: %w", err)
	}
	if len(input.EntryIDs) > 0 {
		positive, negative, unknown := 0, 0, 0
		switch input.Outcome {
		case domainmemory.SocialFeedbackPositive:
			positive = 1
		case domainmemory.SocialFeedbackNegative:
			negative = 1
		case domainmemory.SocialFeedbackUnknown:
			unknown = 1
		}
...
WHERE character_id = $1 AND conversation_id = $2 AND id = ANY($3)`,
		...
		if err != nil {
			return domainmemory.SocialReplyFeedback{}, fmt.Errorf("updating social memory feedback counters: %w", err)
		}
		if changed.RowsAffected() != int64(len(input.EntryIDs)) {
			return domainmemory.SocialReplyFeedback{}, errors.New("social feedback did not update every referenced entry")
		}
		if _, err := tx.Exec(ctx, `
UPDATE social_memory_entries
...
WHERE character_id = $1 AND conversation_id = $2 AND id = ANY($3)
  ...
  AND negative_count >= positive_count + 2`,
			...
			return domainmemory.SocialReplyFeedback{}, fmt.Errorf("suppressing social memory entries: %w", err)
		}
	}
	return domainmemory.SocialReplyFeedback{
...
func QueryRecentSocialFeedbackSummary(ctx context.Context, db Querier, characterID, conversationID string, limit int) (domainmemory.RecentSocialFeedbackSummary, error) {
	rows, err := db.Query(ctx, `
...
LIMIT $3`, characterID, conversationID, limit)
	if err != nil {
		return domainmemory.RecentSocialFeedbackSummary{}, fmt.Errorf("querying recent social feedback: %w", err)
	}
	defer rows.Close()
	var summary domainmemory.RecentSocialFeedbackSummary
	for rows.Next() {
		var outcome string
		var observed int
		if err := rows.Scan(&outcome, &observed); err != nil {
			return domainmemory.RecentSocialFeedbackSummary{}, fmt.Errorf("scanning recent social feedback: %w", err)
		}
		...
		default:
			return domainmemory.RecentSocialFeedbackSummary{}, errors.New("stored social feedback outcome is invalid")
		}
		if observed < 0 {
			return domainmemory.RecentSocialFeedbackSummary{}, errors.New("stored social feedback observation count is invalid")
		}
		summary.SampleCount++
		summary.ObservedMessageCount += observed
	}
	if err := rows.Err(); err != nil {
		return domainmemory.RecentSocialFeedbackSummary{}, fmt.Errorf("iterating recent social feedback: %w", err)
	}
	return summary, nil
}
