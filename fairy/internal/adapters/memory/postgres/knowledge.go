...
func ListKnowledge(ctx context.Context, db Querier, status string) ([]domainmemory.KnowledgeRecord, error) {
	...
	defer rows.Close()
	records := make([]domainmemory.KnowledgeRecord, 0)
	for rows.Next() {
...
func KnowledgeByID(ctx context.Context, db ConversationDB, id string) (domainmemory.KnowledgeRecord, error) {
...
func ScanKnowledge(ctx context.Context, db Querier, row scanner) (domainmemory.KnowledgeRecord, error) {
	var record domainmemory.KnowledgeRecord
	var confidence int
	...
	if err := row.Scan(&record.ID, &record.Topic, &record.Statement, &record.Status, &record.VerificationBasis, &confidence, &record.SourceConversationID, &record.SourceTurnID, &supersedes, &record.CreatedAtUnixMS, &record.UpdatedAtUnixMS); err != nil {
		return domainmemory.KnowledgeRecord{}, fmt.Errorf("scanning knowledge: %w", err)
	}
	if confidence < 0 || confidence > 10000 {
		return domainmemory.KnowledgeRecord{}, errors.New("knowledge confidence is invalid")
	}
	...
	if err != nil {
		return domainmemory.KnowledgeRecord{}, err
	}
	record.Sources = sources
	return record, nil
}

func KnowledgeSources(ctx context.Context, db Querier, id string) ([]domainmemory.AssistantSource, error) {
	...
	defer rows.Close()
	sources := make([]domainmemory.AssistantSource, 0)
	for rows.Next() {
		var source domainmemory.AssistantSource
		var rank int
...
func InsertKnowledgeSource(ctx context.Context, tx pgx.Tx, knowledgeID, sourceID string, source domainmemory.AssistantSource) error {
