...
func validateTurnRuntimeEventInput(input TurnRuntimeEventInput) error {
	if err := domainmemory.ValidateID("conversation_id", input.ConversationID); err != nil {
		return err
	}
	if err := domainmemory.ValidateID("turn_id", input.TurnID); err != nil {
		return err
	}
...
func validateLaneContinuation(record LaneContinuationRecord) error {
	if err := domainmemory.ValidateID("conversation_id", record.ConversationID); err != nil {
		return err
	}
...
func validateContextWindow(record ContextWindowRecord) error {
	...
	if err := domainmemory.ValidateID("first_window_id", record.FirstWindowID); err != nil {
		return err
	}
	if record.PreviousWindowID != nil {
		if err := domainmemory.ValidateID("previous_window_id", *record.PreviousWindowID); err != nil {
			return err
		}
	}
	if err := domainmemory.ValidateID("window_id", record.WindowID); err != nil {
		return err
	}
...
func validatePromptLane(lane string) error {
	if lane == "" || strings.TrimSpace(lane) != lane || domainmemory.ContainsDisallowedControl(lane) {
...
func validateRuntimeToken(label string, value string) error {
	if value == "" || strings.TrimSpace(value) != value || domainmemory.ContainsDisallowedControl(value) || len([]rune(value)) > maxRuntimeTokenRunes {
