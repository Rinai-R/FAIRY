...
func (s *Store) failExtractionBatchPostgres(ctx context.Context, batchID, code, message string, retryable bool) error {
	if err := domainmemory.ValidateID("batch_id", batchID); err != nil {
		return err
	}
	if code == "" || message == "" {
...
func retrievePersonalExtractionProjectionPostgres(ctx context.Context, db Querier, characterID string, projection []string, remaining *int) ([]RetrievedPersonalMemory, error) {
	return RetrievePersonalExtractionProjection(ctx, db, characterID, projection, remaining, normalizePostgresSearchQuery)
}

func scanRetrievedPersonalMemoriesPostgres(rows interface {
	Close()
	Next() bool
	Scan(dest ...any) error
	Err() error
}, remaining *int) ([]RetrievedPersonalMemory, error) {
	// pgx.Rows satisfies ScanRetrievedPersonalMemories; keep wrapper for same-package callers.
	if pgxRows, ok := rows.(pgx.Rows); ok {
		return ScanRetrievedPersonalMemories(pgxRows, remaining)
	}
	return nil, errors.New("scanning retrieved personal memories requires pgx rows")
}
