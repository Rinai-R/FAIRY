...
func CommitPromptWindow(
	...
	summary string,
	now int64,
) (domainmemory.CompactionResult, error) {
	if err := RequireConversation(ctx, tx, conversationID); err != nil {
		return domainmemory.CompactionResult{}, err
	}
	...
	return domainmemory.CompactionResult{WindowRevision: uint64(nextRevision), RetainedDialogueItems: 0}, nil
}
...
func CommitCompaction(
	...
	expectedRevision, nextRevision int64,
	summary string,
	contextWindow domainmemory.ContextWindowRecord,
	clearLane string,
	now int64,
) (domainmemory.CompactionResult, error) {
	if err := RequireConversation(ctx, tx, conversationID); err != nil {
	...
	return domainmemory.CompactionResult{WindowRevision: uint64(nextRevision)}, nil
}
