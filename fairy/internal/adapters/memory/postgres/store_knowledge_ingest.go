...
const maxKnowledgeIngestJobsPerPass = MaxKnowledgeIngestJobsPerPass

type knowledgeIngestJob = KnowledgeIngestJob
...
func (s *Store) enqueueKnowledgeIngestSnapshotsPostgres(ctx context.Context, snapshots []KnowledgeIngestSnapshot) error {
	...
	for _, snapshot := range snapshots {
		...
		if err := domainmemory.ValidateID("conversation_id", snapshot.ConversationID); err != nil {
			return err
		}
		if err := domainmemory.ValidateID("turn_id", snapshot.TurnID); err != nil {
			return err
		}
