...
func (s *Store) ListSocialPersonNotes(ctx context.Context, characterID, conversationID string, senderIDs []string) ([]SocialPersonNote, error) {
	...
	for _, id := range senderIDs {
		...
		if err := domainmemory.ValidateID("sender_id", id); err != nil {
			return nil, err
		}
		if _, ok := seen[id]; ok {
			continue
...
func validateSocialPersonNoteInput(input SocialPersonNoteInput) error {
	if err := domainmemory.ValidateID("character_id", input.CharacterID); err != nil {
		return err
	}
	if err := domainmemory.ValidateID("conversation_id", input.ConversationID); err != nil {
		return err
	}
	if err := domainmemory.ValidateID("sender_id", input.SenderID); err != nil {
		return err
	}
