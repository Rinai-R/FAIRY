...
func TestSocialMemoryContentHashIsStableAndKindSeparated(t *testing.T) {
	entry := domainmemory.SocialMemoryEntryInput{
		Kind: domainmemory.SocialMemoryExpression, Situation: "群友用反讽方式夸张吐槽时",
	...
	entry.Kind = domainmemory.SocialMemoryBehavior
...
func TestBuildSocialRetrievalProjectionSamplesLongQueryAcrossItsRange(t *testing.T) {
	projection := buildSocialRetrievalProjection(strings.Repeat("前", domainmemory.MaxFTSQueryChars-4) + "结尾主题")
