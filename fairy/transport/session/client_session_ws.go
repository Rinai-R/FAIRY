...
type sessionClientFrame struct {
	...
	Interaction        Context                   `json:"interaction,omitempty"`
...
type sessionServerFrame struct {
	...
	CaptureRequest *DesktopCaptureRequest `json:"captureRequest,omitempty"`
	cause          error
}
...
type SessionSocket struct {
	...
	openedConversations        map[string]EndpointKind
	requestCapacity            int
	watchCapacity              int
...
func (c *Client) DialSession(ctx context.Context) (*SessionSocket, error) {
	...
	socket := &SessionSocket{
		...
		openedConversations:        make(map[string]EndpointKind),
...
func (s *SessionSocket) readLoop() {
	...
	for {
		...
		case "participation.event":
			if frame.Participation == nil {
				continue
			}
			conversationID := frame.ConversationID
			if conversationID == "" {
				conversationID = frame.Participation.ConversationID
			}
			s.mu.Lock()
			ch := s.participationEvents[conversationID]
			if ch == nil {
				s.mu.Unlock()
				continue
			}
			select {
			case ch <- *frame.Participation:
			default:
				s.mu.Unlock()
				terminalErr = fmt.Errorf("%w: conversation %s", ErrParticipationConsumerOverflow, conversationID)
				return
			}
			s.mu.Unlock()
		case "desktop.capture.request":
			...
			s.mu.Unlock()
			if !opened || endpoint != EndpointDesktop {
...
func (s *SessionSocket) ParticipationEvents(conversationID string) (<-chan ParticipationEvent, error) {
	...
	defer s.mu.Unlock()
	if s.closed || s.closing {
		return nil, errSessionSocketClosed
	}
	ch := s.participationEvents[conversationID]
	if ch == nil {
