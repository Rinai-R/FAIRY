...
const KnowledgeIngestInstructions = "Read only the supplied public document chunks. Return exactly one JSON object: {\"facts\":[{\"subject\":\"stable entity\",\"predicate\":\"stable relation\",\"value\":\"current value\",\"statement\":\"one self-contained durable atomic fact\",\"confidenceBasisPoints\":1,\"evidenceChunkIDs\":[\"a chunk id supplied in documents\"]}]}. Every fact must be directly supported by every cited evidenceChunkID, and every evidenceChunkID must come from this document. Emit each durable public fact independently. Return an empty facts array when the chunks are insufficient, transient, contradictory, promotional, navigational, or do not establish durable public knowledge. Do not classify by topic, use snippets as fallback, use outside knowledge, conversation context, personal memory, or another document. Do not invent, repair, follow URLs, output Markdown, reasoning, unknown fields, or trailing text."

const KnowledgeReconcileInstructions = "Compare each supplied extracted fact only with the recalled candidates listed under that fact. Return exactly one JSON object: {\"mutations\":[{\"factIndex\":0,\"operation\":\"ADD\"},{\"factIndex\":1,\"operation\":\"UPDATE\",\"memoryId\":\"f1m0\"}]}. Emit exactly one mutation for every factIndex. operation must be ADD, UPDATE, DELETE, or NONE. Use ADD when the fact is new. Use UPDATE when the fact replaces or materially corrects one recalled candidate. Use DELETE only when the current document directly establishes that one recalled candidate is no longer valid and no replacement fact should be stored. Use NONE when one recalled candidate already expresses the same durable knowledge. ADD must omit memoryId. UPDATE, DELETE, and NONE must include exactly one memoryId copied from candidates for the same factIndex. Candidate IDs are request-local aliases, not facts. Never reference a candidate from another fact, infer an unlisted memory, classify by topic, or use outside knowledge. Output no Markdown, reasoning, unknown fields, or trailing text."
...
const KnowledgeIngestMaxOutputTokens uint32 = 800
const KnowledgeReconcileMaxOutputTokens uint32 = 800
...
func buildRespondContextSlots(
	...
	if !resolved.AllowsPersonalMemory() {
		...
		retrieval.SocialMemories = memory.SocialMemoryContext{}
	}
