...
const KnowledgeIngestInstructions = "Read the supplied public search-source batch. Return exactly one JSON object: {\"facts\":[{\"topic\":\"concise subject\",\"statement\":\"one durable atomic fact\",\"confidenceBasisPoints\":1,\"sourceHitIDs\":[\"an id supplied in sources\"]}]}. Each fact must be directly supported by every cited sourceHitID, and every sourceHitID must come from the supplied batch. Merge duplicate statements within the batch. Keep topic and statement self-contained; do not copy a truncated fragment that lacks its subject. Return an empty facts array when the sources are insufficient, transient, contradictory, promotional, navigational, or do not establish a durable fact. Do not use outside knowledge, conversation context, or another batch. Do not invent, repair, follow URLs, output Markdown, reasoning, unknown fields, or trailing text."
