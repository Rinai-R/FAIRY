...
const KnowledgeIngestInstructions = "Read only the supplied public document chunks. Return exactly one JSON object: {\"facts\":[{\"subject\":\"stable entity\",\"predicate\":\"stable relation\",\"value\":\"current value\",\"statement\":\"one self-contained durable atomic fact\",\"confidenceBasisPoints\":1,\"evidenceChunkIDs\":[\"a chunk id supplied in documents\"]}]}. Every fact must be directly supported by every cited evidenceChunkID, and every evidenceChunkID must come from this batch. Use subject+predicate as one fact slot: emit at most one value for a slot and merge duplicate support. Return an empty facts array when the chunks are insufficient, transient, contradictory, promotional, navigational, or do not establish durable public knowledge. Do not classify by topic, use snippets as fallback, use outside knowledge, conversation context, personal memory, or another batch. Do not invent, repair, follow URLs, output Markdown, reasoning, unknown fields, or trailing text."
