...
existing="$(psql "$FAIRY_DATABASE_URL" -Atqc "SELECT count(*) FROM pg_tables WHERE schemaname = current_schema()")"
...
psql "$FAIRY_DATABASE_URL" -v ON_ERROR_STOP=1 -Atqc "SELECT count(*) FROM pg_tables WHERE schemaname = current_schema()"
