...
## P0：对话 workflow → 角色决策 agent loop

**目标**：把现有对话循环收敛成单调用内部角色决策 agent loop，并以有限状态机组织阶段；不引入工具型 ReAct、Reflection 重试或第二次模型调用。

**当前状态（2026-07）：已实现。** Respond 在一次模型请求内先内部选择角色立场、回复意图、语气、关系信号与展开程度；模型执行和严格 reply 编译位于 `planning`，成功后才进入 `responding`。外部仍只输出角色台词的 JSON reply chains，不保存或暴露 decision、思维链和工具轨迹。

Agent 沿状态机思考，尽量推测当前角色的性格、特征、口癖与关系状态，据此预测：面对某条用户消息时，这个角色会采取什么决策、用什么方式回复。

- 状态机阶段应对齐现有「理解语境 → 结合人设/记忆 → 生成可说出口的下一句」工作流，而不是工具调用工作流。
- 输出仍是真人感聊天，不是计划书、工具轨迹或系统旁白。
- 实现与验收见 `openspec/changes/add-character-decision-agent-loop/`；后续调整不得恢复第二次 Interpreter 调用或把内部 decision 暴露给 UI/存储。
